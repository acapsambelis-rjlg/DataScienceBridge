...
namespace DataScienceWorkbench
{
    public class MainForm : Form
    {
        ...
        private void InitializeComponents()
        {
            ...
            fileMenu.DropDownItems.Add("Re-export Data", null, (s, e) => { ExportAllData(); SetStatus("Data re-exported successfully."); });
            fileMenu.DropDownItems.Add(new ToolStripSeparator());
            ...
            helpMenu.DropDownItems.Add("About", null, (s, e) => MessageBox.Show(
                ...
                "A .NET Windows Forms application with\n" +
            ...
            statusLabel = new ToolStripStatusLabel("Ready | Python: " + pythonRunner.GetPythonPath());
            ...
            this.KeyDown += (s, e) => { if (e.KeyCode == Keys.F5) { OnRunScript(s, e); e.Handled = true; } };
        }
        ...
        private void OnRunScript(object sender, EventArgs e)
        {
            ...
            SetStatus(result.Success ? "Script completed successfully." : "Script failed with errors.");
        }
        ...
        private void OnInstallPackage(object sender, EventArgs e)
        {
            ...
            if (result.Success)
            {
                ...
                SetStatus(pkg + " installed successfully.");
            }
            else
            {
                AppendOutput("Failed to install " + pkg + ":\n" + result.Error + "\n", Color.FromArgb(255, 100, 100));
                SetStatus("Installation failed for " + pkg);
            }
        ...
        private void OnUninstallPackage(object sender, EventArgs e)
        {
            ...
            if (result.Success)
            {
                ...
                SetStatus(pkg + " uninstalled.");
            }
            else
            {
                AppendOutput("Failed to uninstall " + pkg + ":\n" + result.Error + "\n", Color.FromArgb(255, 100, 100));
                SetStatus("Uninstall failed for " + pkg);
            }
            OnRefreshPackages(null, null);
        }
...
TIPS:
  ...
  - Install any pip package via Package Manager";
        ...
        {
            statusLabel.Text = msg + " | Python: " + pythonRunner.GetPythonPath();
        }
