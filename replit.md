...
- **In-memory Data Bridge:** All .NET data is streamed directly into the Python environment as pre-loaded variables, eliminating file I/O for data transfer. Supports recursive flattening of nested `[UserVisible]` classes into prefixed DataFrame columns (e.g., `Address_City`, `Address_State`).
...
## Recent Changes
- 2026-02-16: Implemented recursive flattening of nested [UserVisible] classes. The [UserVisible] attribute now supports class-level decoration (AttributeTargets.Class) to signal that a class's properties should be explored and flattened. New FlattenedProperty class and GetFlattenedProperties() method walk the object graph recursively (up to depth 4), producing prefixed column names (e.g., Address_Street, Address_City). Updated CSV serialization, Data Reference tree, field detail panel, autocomplete, and search filter to all use flattened properties. Added [UserVisible] to Address class and its properties as demo.
