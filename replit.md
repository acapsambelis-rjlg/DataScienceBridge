...
- **Unified Streaming API (`_IntelliSEMStream`):** Streaming datasets (`DataQueue`) now expose the same API as in-memory datasets (`_IntelliSEMDataset`). Supports: `__iter__`, `__len__`, `__getitem__` (int, slice, string column, list-of-strings, boolean filter), `.columns`, `.df`, `.head(n)`, `.tail(n)`, `.describe()`, `.to_df()`, column attribute access (`stream.ColName`), column aggregations (`.mean()`, `.sum()`, `.min()`, `.max()`, `.value_counts()`), boolean filters (`stream[stream.Area > 100]`), composite filters (`&`, `|`). Multi-pass re-streaming via `__RESTREAM__:name` stdout protocol when `DataQueue.SetSource(Func<IEnumerable<T>>)` provides a factory. Numeric type coercion is applied during row parsing.
