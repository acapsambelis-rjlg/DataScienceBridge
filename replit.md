...
- **Python Helper Functions:** The `python/helpers/` directory contains `.py` files with reusable helper functions that are automatically loaded into the `DotNetData` module at bootstrap. Users import them via `from DotNetData import display_images`. Adding new helpers is done by creating new `.py` files — no C# changes required. Public functions (not starting with `_`) are auto-discovered. Built-in helpers include `display_images`, `display_image`, `compare_images`, and `image_stats`. Autocomplete discovers helper function names by scanning `def` declarations in the `.py` files.
