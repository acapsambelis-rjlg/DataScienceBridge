...
- Mono project uses DockPanel as root container with `ShowDocumentIcon = true`. The Python editor is a fixed Document-area panel. Files, Output, Data Reference, and Package Manager are all draggable/dockable ToolDockContent panels that can be repositioned, floated, tabbed together, or hidden. All tabs are auto-hidden by default (collapsed to edge strips, slide out on click) and display relevant icons (folder for Files, terminal for Output, graph for Data Reference, box for Package Manager, code brackets for Editor).
