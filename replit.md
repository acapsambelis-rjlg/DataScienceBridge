...
- **Python Helper Functions:** Built-in helper functions are embedded in the bootstrap via `AppendHelperFunctions()` in `PythonBridge.cs` and attached to the `DotNetData` module. Users import them via `from DotNetData import display_images`. Available helpers: `display_images` (image grid), `display_image` (single image), `compare_images` (side-by-side), `image_stats` (RGB statistics). Helper names are listed in `PythonRunner.BuiltInHelperNames` and fed to the completion provider for autocomplete.
