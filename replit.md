...
- **Integrated Python Editor:** Features syntax highlighting, line numbers, real-time syntax checking, code snippets, context-aware autocomplete (for dataset columns, class members, dynamically introspected package members), bracket matching, code folding, bookmarks, find/replace, and error squiggles with tooltips. PythonSymbolAnalyzer uses dynamic symbol loading — known modules, member names, and magic names are instance fields initialized with sensible defaults, with public API for Add/Remove/Set/LoadModuleSymbols. `LoadSymbolsFromVenv(path)` auto-discovers installed packages from the venv's site-packages directory. Symbols are refreshed automatically when the venv initializes and after each successful pip install.
- **Dynamic Module Introspection:** `python/introspect_modules.py` uses Python's `inspect` module to discover all public functions, classes, constants, and submodules of any installed package at runtime. PythonBridge runs this script asynchronously via `IntrospectModulesAsync()` after venv initialization and after each pip install. Results are parsed by the hand-rolled JSON parser in `ModuleIntrospection` (no external JSON dependency) and fed to `DataSciencePythonCompletionProvider.SetModuleCompletions()`. This replaces the previous hardcoded lists (PandasMethods, NumpyMethods, MatplotlibMethods, DataFrameMethods) with version-accurate completions from the actual installed packages. Common import aliases (pd→pandas, np→numpy, plt→matplotlib.pyplot, etc.) are managed via `CommonImportAliases` in DataScienceControl and `SetImportAliases()` on the completion provider. The introspection script has a `PROMINENT_CLASSES` filter to limit class introspection to user-facing types (e.g., DataFrame, Series, ndarray) and caps class discovery at `MAX_CLASSES=50` per module.
...
- **Interactive Script Execution:** Scripts run asynchronously with real-time streaming output. The output panel becomes an interactive terminal during execution — users can type responses to `input()` prompts and press Enter to send. Ctrl+C (with no text selected) sends SIGINT to cancel the running script. The Run button toggles to a Stop button during execution. Input position tracking preserves user-typed text even when new output arrives concurrently. Edit guards prevent modifying historical output above the input prompt. Ctrl+A selects only the current input text.
- **Run Configurations:** PyCharm-style run configuration system with a toolbar dropdown for quick switching. Each configuration can specify: a target script (or "Current File" mode), command-line arguments (passed as `sys.argv`), and an input file whose lines are piped to `input()` calls sequentially. Configurations are persisted in `python/run_configurations.ini` using a simple INI-like format with `[config]` sections and a `[selected]` section for remembering the active selection. The "Edit Configurations..." dialog allows adding, removing, duplicating, and editing configurations with script/input file browsing. `RunConfigurationDialog` is in `src/RunConfigurationDialog.cs`. The config dropdown is disabled during script execution.
- **In-memory Data Bridge:** Streams .NET data to Python via a virtual `DotNetData` module, eliminating file I/O. Supports recursive flattening of nested `[PythonVisible]` classes into prefixed DataFrame columns and serializes Bitmap/Image properties to PIL Image objects in Python. Uses `IInMemoryDataSource` interface for streaming: existing `Func<List<T>>` and `DataTable` registrations wrap results in `StringDataSource`, while `DataQueue<T>` provides memory-efficient row-by-row streaming (items are dequeued and serialized one at a time during transfer, freeing memory progressively). The Python-side protocol (`__DATASET__||name||linecount` + CSV lines + `__DONE__`) is unchanged — both source types produce identical stdin output.
