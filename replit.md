...
- **In-memory Data Bridge:** All .NET data is streamed into the Python environment via a virtual `DotNetData` module, accessed with `from DotNetData import customers, employees`. Eliminates file I/O for data transfer. Supports recursive flattening of nested `[PythonVisible]` classes into prefixed DataFrame columns (e.g., `Address_City`, `Address_State`). Bitmap/Image properties are automatically serialized as base64 PNG and decoded into PIL Image objects in Python.
...
## Recent Changes
- 2026-02-17: Datasets now require explicit import via `from DotNetData import customers, employees` instead of being injected as global variables. A virtual Python module `DotNetData` is created in `sys.modules` during the preamble. Symbol analyzer flags unimported dataset names. Autocomplete suggests dataset names after `from DotNetData import` and supports `DotNetData.` dot-completion. All snippets, default script, help text, Data Reference example code, and dataset detail panels updated. Both codebases synced.
- 2026-02-17: Added row indexing and slicing to `_DotNetDataset` wrapper. `customers[0]` returns a `_DatasetRow` with attribute access (e.g., `customers[0].Name`). `customers[0:5]` returns a new dataset. `for c in customers:` iterates rows. Autocomplete recognizes `dataset[index].` patterns and suggests column names. Updated example code, help text, and default script to use new syntax.
- 2026-02-17: Package Manager tab now loads packages asynchronously on a background thread. Package list is pre-loaded at startup (after venv setup), so it's usually ready before the user opens the tab. Shows "Loading packages..." indicator while fetching. Refresh button also uses async loading. No more UI freeze when opening the Package Manager tab for the first time.
- 2026-02-17: Fixed syntax highlighting bug on Windows where single-line string regex patterns matched across line boundaries due to `\r\n` line endings. Updated all string regex character classes to exclude `\r` and `\n`, preventing cross-line matching and inverted coloring.
