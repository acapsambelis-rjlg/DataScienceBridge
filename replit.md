...
## Recent Changes
- 2026-02-13: Added grouped package list with owner-drawn rendering: Base Packages, User Installed, and Dependencies categories. Uses `packaging.requirements.Requirement` and `importlib.metadata` for accurate transitive dependency tracing with proper name canonicalization and PEP-508 marker evaluation. Fixed pip commands failing inside venv due to Nix's global `PIP_USER=yes` by setting `PIP_USER=0` environment variable override. Added `BasePackageNames` property and `ListPackagesGrouped()` method to PythonBridge.
