...
- **Data Reference Panel:** Displays loaded datasets, columns, data types (including nullable indicators, enum value listings, and sorted dictionary key/value types), registered Python classes, and context variables. `GetPythonTypeName` handles `Nullable<T>` (appends "(nullable)"), enums (lists all values), and `SortedList<K,V>` (shows "dict (sorted, K → V)"). `ShowFieldDetail` renders dedicated sections for nullable fields, enum values, and sorted dictionaries with context-appropriate Python example code.
